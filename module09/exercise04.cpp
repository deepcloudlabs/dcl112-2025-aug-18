...
vector<employee> employees{
    ...
    auto print_employee = [](employee& emp) {
        std::cout << emp << std::endl;
    };
    sort(execution::par,employees.begin(), employees.end(),[](const employee & left, const employee & right) {
        return right.getMSalary() < left.getMSalary();
    });
    for_each(execution::par,employees.begin(), employees.end(),print_employee);
    return 0;
}