#include <iostream>
#include <vector>
#include <parallel/algorithm> // STL (Standard Template Library)
#include <fstream>
#include <functional>
#include <execution>
...
vector<employee> employees{
    ...
    for (const auto& employee : employees) { // external loop
        if (employee.getMDepartment() == employee::department_t::it) {
            numberOfITEmployees++;
        }
    }
    cout << "[imperative] Number of employees in IT department: " << numberOfITEmployees << endl;
    // legacy functional programming
    // count_if: Higher-Order Function
    function<bool(employee&)> works_in_it = [](const employee& emp) {
        return emp.getMDepartment() == employee::department_t::it;
    };
                          // internal loop
    numberOfITEmployees = count_if(std::execution::par, employees.begin(), employees.end(),works_in_it);
    cout << "[declarative] Number of employees in IT department: " << numberOfITEmployees << endl;
    return 0;
}